    ...
    >
      <div className={styles.cardHeader}>
        <div className={styles.cardHeaderLeft}>
          <span className={styles.orderNumber}>#{String(order.order_number).padStart(4, '0')}</span>
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.cardBodyRight}>
          <span className={styles.createdAt}>{formatDate(order.created_at)}</span>
...
interface OrderDetailPanelProps {
  ...
  onStatusChange: (id: string, status: OrderStatusType, order: OrderWithItemsType) => void
...
function OrderDetailPanel({ order, onStatusChange, onDelete, onClose, isPending, isPanelOpen }: OrderDetailPanelProps) {
  if (!order) {
    return (
    <div className={`${styles.panel} ${isPanelOpen ? styles.panelVisible : ''}`}>
      <div className={styles.panelEmpty}>
          ...
          <p className={styles.panelEmptyDesc}>Seleccioná un pedido de la lista para ver su detalle.</p>
        </div>
      </div>
    )
  }
    ...
    <div className={`${styles.panel} ${isPanelOpen ? styles.panelVisible : ''}`}>
      ...
      <div className={styles.panelBody}>
        <div className={styles.panelSection}>
          ...
          {order.customer_address && (
            <span className={styles.panelSubValue}>{order.customer_address}</span>
          )}
          {order.customer_phone && (
            <span className={styles.panelSubValue}>📱 {order.customer_phone}</span>
          )}
        </div>
        ...
        <div className={styles.panelSection}>
          ...
          <span className={styles.panelValue}>
            {order.modality === 'delivery' ? '🚗 Delivery' : '🏪 Retiro en local'}
          </span>
        </div>
        ...
        <div className={styles.panelSection}>
          ...
          <div className={styles.timestamps}>
            ...
            {order.completed_at && (
              <div className={styles.timestampRow}>
                <span className={`${styles.timestampDot} ${styles.timestampDotCompleted}`} />
...
function OrdersPage() {
  ...
  const [orderToDelete, setOrderToDelete] = useState<OrderWithItemsType | null>(null)
  ...
  const handleStatusChange = (id: string, status: OrderStatusType, order: OrderWithItemsType) => {
    if (updateStatus.isPending) return
    updateStatus.mutate({ id, status }, {
      onSuccess: () => {
        if (
          order.customer_phone &&
          STATUS_WA_MESSAGE[status]
        ) {
          const url = STATUS_WA_MESSAGE[status]!(order.customer_name, order.customer_phone)
          setPendingWaUrl(url)
        }
      }
    })
  ...
  const handleDeleteRequest = (order: OrderWithItemsType) => setOrderToDelete(order)
          ...
          <div className={styles.filterSection}>
            ...
            <div className={styles.filterGroup}>
              {(['all', 'pickup', 'delivery'] as const).map((m) => (
                ...
                >
                  {m === 'all' ? 'Todas' : m === 'pickup' ? 'Retiro' : 'Delivery'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className={styles.list}>
            {filtered.map((order) => (
              <OrderCard
                ...
                onClick={() => { setSelectedOrderId(order.id); setIsPanelOpen(true) }}
              />
            ))}
          </div>
        )}
